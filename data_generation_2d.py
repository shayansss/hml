...
jobName, partName, instanceName, modelName = 'PlaneStrain', 'PlaneStrain', 'PlaneStrain-1', 'PlaneStrain'
...
for u1, u2, c10, timeSpan in zip(u1List, u2List, c10List, timeSpanList):
	...
	modelObj.steps['UC'].setValues(timePeriod=timeSpan, initialInc=timeSpan/10, maxInc=timeSpan/10)
	...
	PLANESTRAIN = odb.rootAssembly.instances['PLANESTRAIN-1']
	...
	GAGarrays = frameObjT.fieldOutputs['SDV6'].getSubset(region = PLANESTRAIN, position = ELEMENT_NODAL).values
	# PORarrays = frameObjT.fieldOutputs['POR'].getSubset(region = PLANESTRAIN_HYPERELASTIC, position = NODAL).values
	Sarrays = frameObjT.fieldOutputs['S'].getSubset(region = PLANESTRAIN_HYPERELASTIC, position = ELEMENT_NODAL).values
	LEarrays = frameObjT.fieldOutputs['LE'].getSubset(region = PLANESTRAIN_HYPERELASTIC, position = ELEMENT_NODAL).values
	
	temp = []
	for (GAG, S, LE) in zip(GAGarrays, Sarrays, LEarrays):
		temp.append(np.array([GAG.data, S.data[0], S.data[1], LE.data[0], LE.data[1], i], dtype="float32"))
