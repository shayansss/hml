...
def _parse(proto, meta):
  ...
  for key, field in meta['features'].items():
    ...
    data = tf.reshape(data, field['shape']) # initially they are 1D arrays.
    # for static data is saved one time so will be repeated by tf.tile for each frame
    if field['type'] == 'static': 
      data = tf.tile(data, [meta['trajectory_length'], 1, 1])
    elif field['type'] == 'dynamic_varlen': # ??? Not important as for now we focus on initial and final trajectories
    ...
    out[key] = data
  return out # output is a dict that with reshaped values. Also, the first dim is trajectory
...
def split_and_preprocess(ds, noise_field, noise_scale, noise_gamma):
  ...
  ds = ds.flat_map(tf.data.Dataset.from_tensor_slices) # e.g., [1,2,3] ==>> [1],[2],[3] for each prev, target, ...
