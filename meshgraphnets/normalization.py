...
class Normalizer(snt.AbstractModule):
  ...
  def _std_with_epsilon(self): # edited
    safe_count = tf.maximum(self._acc_count, 1.)
    variance = self._acc_sum_squared / safe_count - self._mean()**2
    variance_corrected = tf.where(variance < 0, tf.zeros_like(variance, dtype=tf.float32), variance)
    std = tf.sqrt(variance_corrected)
